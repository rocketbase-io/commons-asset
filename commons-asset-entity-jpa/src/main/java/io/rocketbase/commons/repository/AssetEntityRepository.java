...
public interface AssetEntityRepository extends PagingAndSortingRepository<AssetJpaEntity, String>, JpaSpecificationExecutor<AssetJpaEntity> {
    ...
    @Query("select a from AssetJpaEntity a left join fetch a.keyValues")
    Iterable<AssetJpaEntity> findAll();

    @Query("select a from AssetJpaEntity a left join fetch a.keyValues where a.id = :id")
    Optional<AssetJpaEntity> findById(@Param("id") String id);

    @Query("select a from AssetJpaEntity a left join fetch a.keyValues where a.systemRefId = :systemRefId")
    ...
    @Query(value = "select a from AssetJpaEntity a left join fetch a.keyValues",
